...
class SpHeteGAT_multi(BaseGAttN):
    def inference(inputs_list, nb_classes, nb_nodes, training, attn_drop, ffd_drop,
                  bias_mat_list, hid_units, n_heads, activation=tf.nn.elu, residual=False,
                  mp_att_size=128):
        embed_list = []
        for inputs, bias_mat in zip(inputs_list, bias_mat_list):
            attns = []
            for _ in range(n_heads[0]):
                attns.append(layers.sp_attn_head(inputs, out_sz=hid_units[0], adj_mat=bias_mat,
                                              activation=activation, nb_nodes=nb_nodes,
                                              in_drop=ffd_drop, coef_drop=attn_drop, residual=False))
            h_1 = tf.concat(attns, axis=-1)
            ...
            for i in range(1, len(hid_units)):
                ...
                attns = []
                for _ in range(n_heads[i]):
                    attns.append(layers.sp_attn_head(h_1, out_sz=hid_units[i], adj_mat=bias_mat,
                                                  activation=activation, nb_nodes=nb_nodes,
                                                  in_drop=ffd_drop, coef_drop=attn_drop, residual=residual))
                h_1 = tf.concat(attns, axis=-1)
            embed_list.append(tf.expand_dims(tf.squeeze(h_1), axis=1))
